...
namespace BetterYoutubeBoombox.Managers
{
    public class BoomboxController : NetworkBehaviour
    {
        ...
        public BetterBoomboxUIManager UI;
        ...
        {
            if (StartOfRound.Instance != null && Instance != null && StartOfRound.Instance.localPlayerController.currentlyHeldObjectServer == Instance.Boombox)
            {
                DebugLog($"Boombox button pressed!");

                if (UI == null)
                {
                    UI = Instantiate(AssetLoader.UIPrefab, HUDManager.Instance.HUDContainer.transform).GetComponent<BetterBoomboxUIManager>();
                    //YoutubeBoomboxPlugin.Instance.PrintChildren(UI.transform);
                    //DebugLog(HUDManager.Instance.weightCounter.font.name);
                    UI.SetPanel(HUDManager.Instance.weightCounter.font);
                }
        ...
        public void ToggleBoombox(bool startMusic, bool pitchDown)
        {
            ...
            if (startMusic)
            {
                IncrementPlaylistIndexClientRpc();
                return;
            }
        ...
        public void PlaySong(string url)
        {
            ...
            ParsedUri parsedUri = YoutubeBoomboxPlugin.Providers.First(p => p.Hosts.Contains(uri.Host)).ParseUri(uri);
        ...
        [ClientRpc]
        public void IncrementPlaylistIndexClientRpc()
        {
            if (CurrentId == null)
            {
